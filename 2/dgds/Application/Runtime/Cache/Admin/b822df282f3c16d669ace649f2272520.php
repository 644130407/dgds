                ...
                <div>
                    <table class="table table-border table-bordered">
                        ...
                        <?php if(is_array($pic)): foreach($pic as $key=>$v): ?><tr>
                                ...
                                <td width="10%"><image src="/dgds_6.18/2/dgds/Public/Uploads/<?php echo ($v["path"]); ?>/<?php echo ($v["name"]); ?>" style="width:100px;height: 100px"/></td>
                                <td width="10%"><?php echo (date('y-m-d H:i',$v["create_time"])); ?></td>
                                <td width="15%"><?php echo ($v["link"]); ?></td>
                                <td width="20%">

                                    <a class="btn btn-primary" href="/dgds_6.18/2/dgds/Public/Uploads/<?php echo ($v["path"]); ?>/<?php echo ($v["image"]); ?>">查看详情</a>
                        ...
                        <tr>
                        <form action="<?php echo u('Admin'.'/Picnews/upload');?>" enctype="multipart/form-data" method="post" >
                            <td>输入您想更改的图片id：<input type="text"  name="id" class="input-text radius size-M"/></td>
                            <td> 请输入您想更变的图片链接：<input type="text"  name="link" class="input-text radius size-M"/></td>
                            <td>输入您需要显示的文字：<input type="text"  name="lable" class="input-text radius size-M"/></td> <td></td>
                            <td> 上传您需要显示在首页的图片：<input type="file"  name="photo1" class="input-text radius size-M"/></td>

                            <td><button type="submit" value="upload" class="btn btn-primary">提交</button></td>
                        </form>
                        </tr>
                    </table>
                </div>
